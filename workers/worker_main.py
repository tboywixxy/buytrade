...
async def upload_server_dat(file: UploadFile = File(...)):
    ...
    with open(ACTIVE_SERVERS_DAT, "wb") as f:
        f.write(data)
    return {"ok": True, "saved_as": ACTIVE_SERVERS_DAT, "note": "replaced main servers.dat on this worker", "worker_port": WORKER_PORT}
...
def has_broker(server: str = Query(..., description="MT5 server name to check")):
    ...
    try:
        ...
        return {"ok": True, "has": False, "file_exists": True, "note": "servers.dat unreadable but present"}
    target = server.encode("utf-8")
    if data and target in data:
        return {"ok": True, "has": True, "file_exists": True, "source": "main"}
    return {"ok": True, "has": False, "file_exists": True, "note": "servers.dat present but exact server name not found (binary file)"}
...
def place_order(body: dict = Body(...)):
    ...
    RETCLOSE = {MT5.TRADE_RETCODE_MARKET_CLOSED, MT5.TRADE_RETCODE_NO_PRICES, MT5.TRADE_RETCODE_OFFQUOTES}
    RETVOL = {MT5.TRADE_RETCODE_INVALID_VOLUME, MT5.TRADE_RETCODE_VOLUME_LIMIT}
    RETPRICE = {MT5.TRADE_RETCODE_REQUOTE, MT5.TRADE_RETCODE_PRICE_CHANGED, MT5.TRADE_RETCODE_PRICE_OFF}

    if ret in RETCLOSE:
        raise HTTPException(409, {"error": "market closed/off quotes", "symbol": symbol, "retcode": ret, "detail": detail})
    if ret in RETVOL:
        raise HTTPException(400, {"error": "invalid volume", "symbol": symbol, "requested": volume, "normalized": vol_adj, "limits": [vmin, vmax], "step": vstep, "retcode": ret, "detail": detail})
    if ret in RETPRICE:
        raise HTTPException(409, {"error": "price change/requote", "symbol": symbol, "retcode": ret, "detail": detail})

    raise HTTPException(422, {"error": "order_send failed", "symbol": symbol, "retcode": ret, "detail": detail})
