# worker_main.py
# MT5 Worker (portable + session-safe + idle soft-disconnect)
# ✅ Adds:
#   - last_activity_ts tracking + touch_activity()
#   - /mt5/disconnect (soft disconnect: shutdown + kill terminal, KEEP FILES)
#   - idle janitor (default 240s)
#   - faster base clone via robocopy (fallback to shutil.copytree)
#   - metrics/status expose last_activity_ts
...
app = FastAPI(title="MT5 Worker (portable + session-safe + idle soft-disconnect)", version="1.7.0")
...
# ✅ idle soft-disconnect (keep files)
WORKER_IDLE_TTL_SEC = int(os.environ.get("WORKER_IDLE_TTL_SEC", "240"))  # 4 mins
WORKER_IDLE_POLL_SEC = float(os.environ.get("WORKER_IDLE_POLL_SEC", "15"))  # check interval
# ==================
...
SESSION: Dict[str, Any] = {
    ...
    "last_login_ts": 0.0,
    "last_trade_ts": 0.0,
    "last_activity_ts": 0.0,  # ✅ NEW
}


def touch_activity(reason: str = "activity"):
    # single source of truth for worker-side activity
...
def robocopy_tree(src: str, dst: str, mt: int = 16) -> Tuple[bool, str, int]:
    """
    Returns (ok, output, code).
    Robocopy success codes: 0..7.
    >= 8 is failure.

    /MT speeds up file copy a lot.
    /COPY:DAT avoids ACL issues while preserving timestamps.
    """
    try:
        ...
        return False, f"robocopy exception: {e}", 998


def safe_clone_base(src: str, dst: str) -> None:
    """
    Clone BASE_MT5_DIR -> dst safely:
      - copy into temp folder first
      - move into place atomically
      - fallback to shutil.copytree if robocopy fails
    """
    ...
    tmp_parent = os.path.join(parent, "_tmp")
    os.makedirs(tmp_parent, exist_ok=True)

    tmp_dir = tempfile.mkdtemp(prefix="clone_", dir=tmp_parent)

    try:
        ok, out, code = robocopy_tree(src, tmp_dir, mt=16)
        if not ok:
            # fallback: python copytree
            shutil.rmtree(tmp_dir, ignore_errors=True)
            tmp_dir = tempfile.mkdtemp(prefix="clone_", dir=tmp_parent)
            shutil.copytree(src, tmp_dir, dirs_exist_ok=True)

        # move into place (same volume) - near-atomic
...
def ensure_instance_dir_for_user(user_id: str) -> str:
    ...
    if not os.path.isdir(inst_dir):
        ...
        safe_clone_base(BASE_MT5_DIR, inst_dir)
    ...
    for p in psutil.process_iter(["pid", "name", "exe", "cwd", "cmdline"]):
        try:
            ...
            if target_exe and exe and os.path.abspath(exe).lower() == target_exe:
                return p
            if target_dir and (
                (exe and os.path.abspath(exe).lower().startswith(target_dir))
                or (cwd and os.path.abspath(cwd).lower().startswith(target_dir))
            ):
                return p
        except Exception:
            continue
...
# ✅ NEW: soft disconnect (KEEP FILES)
def soft_disconnect_user(user_id: str, reason: str = "manual") -> Dict[str, Any]:
    """
    Soft disconnect:
      - MT5.shutdown()
      - kill terminal processes for this user's instance dir (this worker port)
      - KEEP instance directory on disk
    ...
    return {"ok": True, "soft_disconnected": True, "user_id": user_id, "reason": reason, "inst_dir": inst_dir}
...
# ✅ NEW: idle janitor (soft-disconnect after TTL)
def idle_janitor():
    while True:
        time.sleep(WORKER_IDLE_POLL_SEC)
        try:
            user_id = SESSION.get("user_id")
            if not user_id:
                continue

            last = float(SESSION.get("last_activity_ts") or 0.0)
            if not last:
                continue

            if (time.time() - last) >= float(WORKER_IDLE_TTL_SEC):
                lk = get_user_lock(user_id)
                with lk:
                    # re-check under lock
                    if SESSION.get("user_id") == user_id:
                        last2 = float(SESSION.get("last_activity_ts") or 0.0)
                        if last2 and (time.time() - last2) >= float(WORKER_IDLE_TTL_SEC):
                            soft_disconnect_user(user_id, reason="idle")
        except Exception:
            pass
...
def on_startup():
    ...
    threading.Thread(target=idle_janitor, daemon=True).start()
...
def metrics():
    return {
        ...
        "idle_ttl_sec": WORKER_IDLE_TTL_SEC,
        "now": time.time(),
    }
...
def get_account_info():
    # ✅ counts as activity (optional)
    touch_activity("account-info")
    with MT5_LOCK:
        info = MT5.account_info()
        if info is None:
...
def cleanup_instance(body: dict = Body(default={})):
    """
    HARD cleanup helper (deletes instance dir for this worker port).
    Keep as-is; manager idle should NOT call this.
    """
...
# ✅ NEW: soft disconnect endpoint (manager calls this)
@app.post("/mt5/disconnect")
def mt5_disconnect(body: dict = Body(...)):
    ...
    lk = get_user_lock(user_id)
    with lk:
        return soft_disconnect_user(user_id, reason=reason)


@app.post("/mt5/logout")
def logout(body: dict = Body(...)):
    """
    HARD logout (deletes ALL ports for user) — keep as your explicit delete.
    """
    user_id = body.get("user_id") or body.get("mt5_account_id")
    if not user_id:
        raise HTTPException(400, {"error": "missing_user_id", "message": "user_id is required"})
...
def _ensure_logged_in(user_id: str) -> None:
    with MT5_LOCK:
        ...
        if (
            ...
            or not SESSION.get("inst_exe")
            or not SESSION.get("login")
            or not SESSION.get("password")
        ):
...
# =========================================================
# Modify SL/TP on an OPEN POSITION (does not logout)
# =========================================================
